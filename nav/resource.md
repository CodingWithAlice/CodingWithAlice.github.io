---
layout: page
title: 芝麻绿豆
permalink: /resource/
header-img: "img/home.jpg"
---




- **我的 BliBli主页：[Alice铛铛铛](<https://space.bilibili.com/14608003>)**

  - 主要分享面试视频
  - 学习记录
  - 热爱生活 vlog

## 莱特纳盒子学习法 v2.0（工作模式）

- ① 间隔周期：渐进式增加间隔 + 连续错误惩罚，再结合基础间隔计算
- ② 由网站提供做题界面，记录做题时间、做题时长

## 莱特纳盒子学习法 v1.1（自学模式）

- ① 间隔周期 `BOXId * 7天 + 上次做题时间 solveTime`
- ② 做错的题目 / 新增的BOX1题目：隔天重做

## 莱特纳盒子学习法 v1.0（自学模式）

-   LTN1：一周后需要再次复习；
-   LTN2：两周后需要再次复习；
-   LTN3：三周后需要再次复习；
-   LTN4：四周后需要再次复习...

----

#### changeLog：2024.10.11

#### 盒子分类标准：

-   文章整合期间（建议3-4天完成，不影响 LTN1 的第一次复习）：
    -   所有知识点默认 LTN1，通过检索判断是否已经掌握，已掌握则升级为 LTN2；
    -   若未掌握，则打上 LTN1 的标签，作为下周复习知识卡；

-   所有文章分类完毕后：
    -   新文章一律归为 LTN1；
    -   所有不想要忘记的知识点，都放到盒子里复习

-   计划目标：
    -   1、3-4天完成原本文章分类（得到LTN1、LTN2 两种知识卡盒子）
    -   2、每周六进行盒子清算，第二周复习的量还有一半，会比较累；
    -   3、持续一个月左右，可以将当前200+知识卡陆续分成4个盒子，复习抓取都比较合适
    -   4、持续两个月左右，生成5-6个盒子，文章的数量即使增加，也控制在比较少的数量（相信自己，大部分都是会的，脑袋越用越记得牢）
    -   5、暂时设置到 LTN7 ，4个月一复习是能想到的比较久的复习方式（类似期末考）。

----

#### changeLog：2024.10.19

更新：10.18 测试 LTN1 盒子，6个知识点转入 LTN2 盒子

-   剩余及新增 LTN1 盒子，将于 10.25 测试
-   LTN2 盒子将于 11.1 再次测试

    整合计划比预估花费时间久，进展（27/250）/ 3Day，主要 html + css，下周重点 js + 浏览器原理

#### changeLog：2024.10.25

更新：10.25 测试 LTN1 盒子，13个知识点转入 LTN2 盒子

-   剩余及新增 LTN1 盒子 + 部分 LNT2 盒子，将于 11.1 测试
-   LTN2 盒子将于 11.9 再次测试

    整合计划比预估花费时间久，进展至（55/250）/ 5Day，主要 js 和 红宝书1-10章，下周重点 浏览器原理 + 简历项目复盘

#### changeLog：2024.11.4

更新：11.2 测试
-   2个知识点从 LTN2 降级为 LTN1
-   4个知识点从 LTN2 升级为 LTN3
-   9个知识点从 LTN1 升级为 LTN2
-   10个知识点停留在 LTN1

下周 11.9 测试（无法单独使用标签作为复习筛选 - 暂时本地笔记维护时间线测试）

    进展至（100/250）/ 6Day，主要 浏览器原理+Promise常见手撕题 和 红宝书11-16章，下周重点 HTTP基础知识 + webpack + 简历项目复盘

#### changeLog：2024.11.12

更新：11.9 测试
-   5个知识点从 LTN2 降级为 LTN1
-   10个知识点从 LTN2 升级为 LTN3
-   11个知识点从 LTN1 升级为 LTN2

下周 11.16 测试（无法单独使用标签作为复习筛选 - 暂时本地笔记维护时间线测试）

    进展至（140/250）/ 5Day，主要 HTTP基础知识 + 简历项目复盘 和 红宝书17章，下周重点 webpack + 手撕题

- React 和 Vue2 复习方式还在思考 show me code？

#### changeLog：2024.11.27

更新：11.27 (11.16的测试11.22才完成， 11.25的测试11.27才完成)
-   37个知识点从 LTN2 降级为 LTN1
-   9个知识点从 LTN3 升级为 LTN4
-   23个知识点从 LTN2 升级为 LTN3
-   26个知识点从 LTN1 升级为 LTN2

下周 11.30 测试（无法单独使用标签作为复习筛选 - 暂时本地笔记维护时间线测试）

    进展至（150/250）/ 5Day，主要 webpack + 手撕题4 和 红宝书18章+19章，下周重点 React 框架相关梳理

各种原因，两周的测试时间撞在一起，导致一直在做题，下周会好很多，找回学习状态，加油！

- React 和 Vue2 复习方式还在思考 show me code？

#### changeLog：2024.12.2

更新：12.2
-   2个知识点从 LTN3 降级为 LTN1
-   8个知识点从 LTN3 升级为 LTN4
-   8个知识点从 LTN1 升级为 LTN2

下周 12.7 测试（无法单独使用标签作为复习筛选 - 暂时本地笔记维护时间线测试）

    进展至（150/250）/ 5Day【本周无博客更新】，下周重点 React + 手撕题投入LTN盒子

- 先复习 React，之前看过的书籍、课程、博客整理 + 重启J-table项目作为code实战

#### changeLog：2024.12.13

更新：12.13
-   6个知识点从 LTN2 降级为 LTN1
-   10个知识点从 LTN1 升级为 LTN2
-   10个知识点从 LTN2 升级为 LTN3

下周 12.16 测试

    进展至（150/250）/ 5Day【本周无博客更新】 + 手撕8题，下周重点依旧是 React基础 + 手撕题 + React项目

- LTN 每周50题，题量有点大，每天15题左右/1.5h投入复习，其他时间用于学习其他内容

#### changeLog：2024.12.21

更新：12.18-12.21
-   4个知识点从 LTN3 升级为 LTN4
-   3个知识点从 LTN2 升级为 LTN3
-   9个知识点从 LTN1 升级为 LNT2
-   21个知识点 退回 LTN1

下周 12.25-12.29 测试，每天一个盒子的题目

    进展至（150/250）/ 5Day【本周无博客更新】 + React 井字格、Daily项目完成，下一个的目标是 理论知识点、面试题、React还原原型demo + 算法题继续复盘投入LTN盒子

- 【更新】① LTN 一个盒子的计划比规定题量更合理 ②同时不再采用按自然周安排测试，而是根据上次测试时间安排下次测试时间 -> 更合理 - 解放了自然周中一大半时间在复盘，导致学习计划一直很难开展的问题。
-   整理需求，把 LTN 题目计划做成工具，手动记录实在太浪费时间+混乱了

#### changeLog：2024.12.30

更新：12.25-12.30
-   7个知识点从 LTN4 升级为 LTN5
-   7个知识点从 LTN2 升级为 LTN3
-   10个知识点从 LTN1 升级为 LNT2
-   31个知识点 退回 LTN1

下周 1.5-1.8 测试，每天一个盒子的题目

    进展至（170/250）/ 5Day + Vue-渲染+原理、React-渲染+性能优化+fiber+hooks+错误处理+高阶组件+合成事件+生命周期+setState、两个框架的 Diff 算法比较；下一个的目标是 ①投递简历，通过第三方收集反馈 + ②LTN管理项目需求整理、开发 + ③手撕、数据结构算法继续复盘 + ④面试模拟题 + ⑤hooks 官网实战 投入LTN盒子

- 【LTN 学习方法更新】① 这次记录了下时间和字数：12h57m 12662字 - 强度太大、耗时太长，影响反馈+修正 -> 改为语音输入对比 ② 一定要“纠正”“纠正”“纠正” - 专注、反馈、纠正 - 一步都不能少

#### changeLog：2025.1.17

更新：1.7-1.17
-   7个知识点从 LTN4 升级为 LTN5
-   7个知识点从 LTN3 升级为 LTN4
-   12个知识点从 LTN2 升级为 LNT3
-   21个知识点从 LTN1 升级为 LNT2
-   28个知识点 退回 LTN1

下周期测试，每天10个题目左右

    进展至（180/260）/ 5Day + redux + MySQL + Nextjs + ①简历已定期投递 + ②日报周期落表本地mysql；下一个的目标是 ①日报事项落地本地MongoDB + ②购买服务器、域名 + ③手撕、数据结构算法继续复盘 + ④面试模拟题 + ⑤LTN工具实现管理

- 【LTN 学习方法更新】① 这次记录的做题时间 25h40m - 耗时太长，影响反馈+修正 -> 不再定期开始做题，借用 LTN工具，尽可能每天做题，定期梳理题目范围即可 ② 做题5分钟，纠正20分钟，纠正自己的思考逻辑才是最重要的

#### changeLog：2025.1.26

做题周期：1.19-1.26， 分散到每天做题，LTN①③ 错题重做28题 + LTN①④做题68题 + LTN①④ 错题重做23题 - 共计 119题，22h

- 本周期进展：LTN 分散后，做题压力很小，有空开始整合项目 - 
    -   ①Next_Pro： 每日复盘 时间、事项 都落表，可以查询、更新
    -   ②J-Table ：使用 nestjs  - LTN题库管理 改为 数据库唯一/统一管理，清理笔记+博客相关记录，同时提供 LTN 时间轴、产出指定周期的题目列表、添加新题目/升降题目归属盒子

下周期测试 74题，周期预估在 1.27 - 2.4  

- 【LTN 学习方法更新】① 这次记录的做题时间 22h - 耗时虽然长，但是包含了两次「错题重做」 - 一般错题重做 5h 左右/2-3天，每天花费时间不多，不构成每日主要学习事项 -> 每日重点逐渐切换到项目 ② 写代码不只是为了实现功能，重点是记录、复盘写代码过程中的收获，作为经验增长自己的能力

#### changeLog:2025.2.10

做题周期:1.27-2.10, 分散到每天做题(中间生病停止5天),LTN①⑤ 做题71题 + LTN①⑤ 错题重做23题 - 共计 94题,14h30m

- 本周期进展：-
  - ①Next_Pro: 提供周报数据源统计, 方便写周报 - 减少手工统计时间
  - ②J-Table: 提供指定周期的 LTN 题目列表, 方便周报做下周计划

下周期测试 90题,周期预估在 2.10 - 2.18(在补之前生病5天 delay 的题目)  

- 【LTN 学习方法更新】① 之前优化的每日 LTN 做题优势在生病 delay 期间展示出了强大的灵活性 - LTN 不作为每日学习重点 ② 每日前端学习增加 codeReview 前一天代码的事项 + 项目需求完全按照自我管理需求来 ③ 逐步稳定前两项后, LTN1 的新增也需要关注

#### changeLog:2025.4.2

已有网站批量管理当前周期 [LTN 题目列表](http://codingwithalice.top:4001/)
已有网站批量管理 [周报](http://codingwithalice.top/week) [2月月报](http://codingwithalice.top/month?monthId=2) [3月月报](http://codingwithalice.top/month?monthId=3)
现已迭代到 LTN②①
- 困境1：已开始上班，当前 LTN 周期题目分布强度无法每日完成
- 困境2：当前LTN 周期设置在 10天左右，LTN 的周期在 7 天，每个周期都无法完全按照日期做题

#### 随着执行计划，可修改当前方案
