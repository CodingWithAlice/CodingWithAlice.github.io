...
17. 怎么**改变this指向**？
    ...
    4. new**实例化一个对象**；
    5. 在非严格模式下，如果如果函数没有用作构造函数，而是仅仅作为普通函数使用的话，那么函数中的this是指向window的。在严格模式下，this的值就是undefined。
...
### 20.http和https分别是什么，有什么区别

|            |                             http                             |                            https                             |
| :--------: | :----------------------------------------------------------: | :----------------------------------------------------------: |
|    全称    |                        超文本传输协议                        |               安全套接字层超文本传输协议HTTPS                |
| 最大的区别 |                       明文方式发送内容                       |    在http协议的基础上加入了ssl协议+证书，用来加密传输数据    |
|  端口不同  |                            80端口                            |                           443端口                            |
|  连接方式  |                          连接很简单                          | SSL+HTTP协议构建的可进行加密传输、身份认证的网络协议，更安全 |
...
[不要说什么get，post请求，post请求只是在浏览器上不显示参数，如果我们用一些抓包工具是一样会把http发送请求的内容给抓过来的；

HTTPS协议的主要作用可以分为两种：一种是建立一个信息安全通道，来保证数据传输的安全；另一种就是确认网站的真实性。

ssl层协议：（全称：Secure Sockets Layer **安全套接层**）是一种**安全协议**，SSL在传输层对网络连接进行加密。（对传输层不太明白的可以查一下网络的七层协议）
...
5. HTTP请求中，头部有一个请求，请求头：content-type，发送请求的数据类型
...
51. 有没有学过webpack？配置或者loader了解过吗？
