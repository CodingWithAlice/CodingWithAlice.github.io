...
```js
// 2024.11 观察者模式 EventEmitter - add remove emit once setMax
class EventEmitter() {
    constructor() {
        this.events = {};
        this._max = 10;
...
```js
// 2024.12 模拟实现promise - then catch finally resolve reject all race
const Status = {
    pending: 'pending',
    resolve: 'fulfilled',
    reject: 'rejected'
...
❌ // 9、对，但是「标记 flag 收集副作用链表」是两件事，一个是对有副作用需要「在更新阶段执行」的节点进行标记 subtreeFlags，另一个是 React 会「构建 side-effects-list 用于收集副作用链表」
...
高阶函数-权限校验示例：
const permission  =(WrapComponent, permissionRequired) => {
    return class extends React.Component {
        ...
        componentDidMount() {
            const usePermission = checkPermission(); // 查询
            this.setState({ hasPermission: usePermission === permissionRequired })
        }
        render(){
