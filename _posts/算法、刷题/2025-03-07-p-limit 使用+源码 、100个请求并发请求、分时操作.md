...
// 第二种简写法
const limitedTask = limitFunction(async (index: number) => {
    await new Promise(resolve => setTimeout(resolve, index * 100));
    return `Result ${index}`;
}, {concurrency: 2}); // 全局限制为2个并发

// 创建任务数组，每个任务调用 limitedTask
const tasks2 = Array.from({length: 20}, (_, index) => limitedTask(index));
Promise.all(tasks2).then((results) => console.log('All:', results) );
```

```js
// 之前的简写法是❌的 -> 每次循环都会创建新的 limitFunction 实例 -> 导致并发数量为 20*2
...
- 任务队列管理 queue：用数组存储待执行的任务，当并发任务数量达到限制时，新的任务被添加到队列
- 并发数量限制 concurrency：计数器 - 跟踪正在执行的任务数量；到限制时，新任务被暂停执行
- 任务执行、调度：有任务完成时，计数器 减1；并从队列中取出一个任务执行，计数器再 +1；系统**永远不会真正结束**，直到没有引用
